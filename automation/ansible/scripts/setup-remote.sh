...
useradd -m -s /bin/bash utcddev || true
usermod -aG sudo utcddev || true

# Create project directory
echo "Setting up project directory..."
PROJECT_DIR="/opt/simple-utcd"
mkdir -p $PROJECT_DIR
chown utcddev:utcddev $PROJECT_DIR

# Create log directories
echo "Creating log directories..."
mkdir -p /var/log/simple-utcd
chown utcddev:utcddev /var/log/simple-utcd

# Create config directory
echo "Creating config directory..."
mkdir -p /etc/simple-utcd
chown utcddev:utcddev /etc/simple-utcd
...
echo "2. Run: cd $PROJECT_DIR && sudo -u utcddev ./build.sh"
echo "3. Test with: sudo -u utcddev make test"
