...
Version:        ${VERSION}
Release:        1%{?dist}
Summary:        simple-utcd - A lightweight and secure daemon
License:        Apache-2.0
...
%description
simple-utcd is a modern, secure implementation of the UTC protocol.
...
if [ "$1" -eq 1 ]; then
    ...
    # Create service user
    if ! id utcddev &>/dev/null; then
        useradd -r -s /sbin/nologin -d /var/lib/%{name} -c "%{name} service user" utcddev
    fi
fi
