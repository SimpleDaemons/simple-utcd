#!/bin/bash
# Create DMG installer for simple-utcd
...
VERSION="${1:-1.0.0}"
DMG_NAME="$simple-utcd-${VERSION}"
DMG_TEMP="dmg-temp"
DMG_FILE="${DMG_NAME}.dmg"
APP_NAME="$simple-utcd.app"
PKG_NAME="$simple-utcd.pkg"
...
if [ -f "../assets/icons/$simple-utcd.icns" ]; then
    # Set custom icon (requires additional tools)
    echo "Custom icon available at ../assets/icons/$simple-utcd.icns"
fi
